...
from ott.geocoder.pyramid import views
...
def do_static_config(config):
    ...
    config.add_translation_dirs('ott.geocoder:locale')
    config.add_subscriber('ott.geocoder.locale.subscribers.add_renderer_globals', 'pyramid.events.BeforeRender')
    config.add_subscriber('ott.geocoder.locale.subscribers.add_localizer', 'pyramid.events.NewRequest')
...
def cmdline():
    ''' as an alternate to pserve, you can run this via bin/python ott/geocoder/pyramid_app 
